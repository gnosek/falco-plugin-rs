...
std::shared_ptr<sinsp_plugin> SinspTestDriver::register_plugin(const Api* api, const char* config)
{
    std::scoped_lock m(s_sinsp_lock);
    std::string err;

    auto plugin = m_sinsp.register_plugin(reinterpret_cast<const plugin_api*>(api));
...
void SinspTestDriver::add_filterchecks(const std::shared_ptr<sinsp_plugin>& plugin, const char* source)
{
    std::scoped_lock m(s_sinsp_lock);
...
{
    std::scoped_lock m(s_sinsp_lock);
    m_sinsp.open_plugin(name, config, sinsp_plugin_platform::SINSP_PLATFORM_GENERIC);
    m_sinsp.start_capture();
}
...
std::unique_ptr<std::string> SinspTestDriver::event_field_as_string(const char* field_name, const SinspEvent& event)
{
    std::scoped_lock m(s_sinsp_lock);
